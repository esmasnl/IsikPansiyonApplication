...
namespace ışık_pansiyon_uygulaması_V._1
{
    public partial class FormYeniMusteri : Form
    {
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("insert into MusteriEkle1 (Adi,Soyadi,Cinsiyet,Telefon,Mail,TC,OdaNo,Ucret,GirisTarihi,CikisTarihi) values('" + TxtAdi.Text.ToString() + "','" + TxtSoyadi.Text.ToString() + "','" + comboBox1.SelectedText.ToString() + "','"+MskTxtTelefon.Text+"','"+TxtMail.Text+"','"+TxtKimlikNo.Text+"','"+TxtOdaNo.Text+"','"+TxtUcret.Text+"','"+DtpGirisTarihi.Value.ToString("yyyy-MM-dd")+"','"+DtpCikisTarihi.Value.ToString("yyyy-MM-dd")+"')", baglanti);
            komut.ExecuteNonQuery();
            baglanti.Close();
...
//Data Source = DESKTOP - DN3E80D; Initial Catalog = İsikPansiyon; Integrated Security = True
